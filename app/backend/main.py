...
async def predict(request: PredictionRequest):
    ...
    Risk thresholds follow Practical Guideline of Febrile Neutropenia（FN）Japanese Society of Medical Oncology, 2012:
      - High Risk         : risk_score > 20 % (proba > 0.20)
      - Intermediate Risk : 10 % ≤ risk_score ≤ 20 % (0.10 ≤ proba ≤ 0.20)
      - Low Risk          : risk_score < 10 % (proba < 0.10)
    ...
    dict
        ...
        ``interpretation`` — "High Risk", "Intermediate Risk", or "Low Risk".
        ``level``          — "high", "intermediate", or "low" (machine-readable alias).
    ...
    try:
        ...
        # Three-tier FN risk classification (clinical guideline thresholds)
        if proba > 0.20:
