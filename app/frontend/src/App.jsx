...
export default function App() {
   ...
   *
   * @param {'high'|'intermediate'|'low'} type - Risk profile to load.
   */
  const fillSample = (type) => {
    if (type === 'high') {
      ...
      setFormData(prev => ({ ...prev, LastNEUT: 2500, LastWBC: 6000, AGE: 45,
        MS_WEIGHT: 70, TEMPR: 36.5, TOTAL_GLAS_PREDOSE: 300, diagnoses: ['C18'],
        drugs: { '4291427': 240 } }))
    }
  }
 ...
 * (high / intermediate / low), and a mandatory clinical disclaimer.
 *
 * Risk thresholds: >20% = High Risk | 10–20% = Intermediate Risk | <10% = Low Risk
 *
 * @param {{ risk_score: number, level: string }} result - API response object.
 */
function ResultPanel({ result }) {
  const pct    = (result.risk_score * 100).toFixed(1)
  const level  = result.level  // 'high' | 'intermediate' | 'low'

  // Colour mapping per risk tier
  const colorMap = {
    high:         'var(--red)',
  ...
  return (
    <div>
      <div className="risk-score-block">
        {/* Numeric risk score */}
        <div className="risk-score-value" style={{ color: colorMap[level] }}>
          {pct}<span style={{ fontSize: '1.1rem', fontWeight: 600 }}>%</span>
        </div>
        <p className="risk-score-label-text">FN Risk Score</p>
        {/* Three-tier risk classification badge */}
        <span className={`risk-level-badge ${badgeClass[level]}`}>
          {labelMap[level]}
        </span>
      </div>
